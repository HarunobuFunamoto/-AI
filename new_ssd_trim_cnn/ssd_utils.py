...
class BBoxUtility(object):
    ...
    def decode_boxes(self, mbox_loc, mbox_priorbox, variances):
        ...
        # List of variances for x, y, w, h.  variances=[0.1, 0.1, 0.2, 0.2]　　variancesは分散、変動、相違つまり左上と横縦の長さの比率じやね
        # 分散を利用する理由は一意に値が決まる訳ではないのである程度範囲を持った予測をできるようにするためです。
        #デコードするバウンダリーボックスを求めるため、先ほどの値と分散を利用してデコードしたバウンダリーボックスの中心位置と幅、高さを求める。予測した値は小さいのでexpにより十分な大きさの値に変換する。注意点として分散を考慮している点。予測した中央点と幅、高さは確率的なものを考慮して分散による値のズレを許容するため入れている。
        #中心位置から最小、最大のオフセットのために変換する
        #求めた値を一つのベクトルにまとめる
        #変換した値から0以上かつ1以下の領域のみ返すようにする

        # そもそもconv2dでマルチスケーリングされて予測してるんだから、実際の値で予測のmbox〜は出てこない
        decode_bbox_center_x = mbox_loc[:, 0] * prior_width * variances[:, 0]
        decode_bbox_center_x += prior_center_x
        decode_bbox_center_y = mbox_loc[:, 1] * prior_width * variances[:, 1]
        decode_bbox_center_y += prior_center_y
        #  decode_bbox_widthが枠の横の長さ
        decode_bbox_width = np.exp(mbox_loc[:, 2] * variances[:, 2])
        decode_bbox_width *= prior_width
        #  decode_bbox_heightが枠の縦の長さ
        decode_bbox_height = np.exp(mbox_loc[:, 3] * variances[:, 3])
        decode_bbox_height *= prior_height
        # decode_bbox_center_が枠の縦、横それぞれの線の中間地点。そこから縦、横の半分の長さを引くと左端、枠の半分の長さを足すと右端になる。*0.5は半分にしてる
    ...
    def detection_out(self, predictions, background_label_id=0, keep_top_k=200,
        ...
        """
        #  model.predictしたものがmbox〜になってる[..4..-8..-4..]
        # オフセット（位置）：mbox_loc
        # 確信度 : mbox_conf
        # 各バウンダリーボックス : mbox_priorbox
        #
        ...
        for i in range(len(mbox_loc)):
            ...
            #  ここで枠の座標を求めてるdecode_boxesで座標求めてる。model.predictしたなかののmbox_conf = predictions[:, :, 4:-8]以外をつかって座標だす
            #  mbox_locは上からズラーっとコンボリューションとプーリングしてきたものをconv２DしてFlattenしたもの、mbo_confはonv２DしてFlattenしてPriorBox関数（指定されたサイズとアスペクト比の以前のボックスを生成する）やる
