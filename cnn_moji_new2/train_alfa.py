...
#print(data_train2)
    ...
    def build_model(in_shape):
        ...
        model.add(Convolution2D(64, 3, 3,border_mode='same',init='he_normal',name='conv'))
        model.add(BatchNormalization(axis=-1, momentum=0.99, epsilon=0.001,center=True, scale=True, beta_initializer='zeros', gamma_initializer='ones'))
        model.add(Activation('relu'))
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Dropout(0.7))
...
if __name__ == '__main__':
    ...
    all_moji = 'zxcvbnmasdfghjklqwertyuiopＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴＵＶＷＸＹＺ'.replace('',' ').split()
    #all_moji = ['Ｄ']
    repeat = 1
